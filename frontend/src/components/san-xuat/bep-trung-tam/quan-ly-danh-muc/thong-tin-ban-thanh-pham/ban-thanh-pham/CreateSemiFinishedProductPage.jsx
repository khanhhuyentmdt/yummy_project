...
export default function CreateSemiFinishedProductPage({ onCancel, onSaved }) {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const res = await api.post("semi-finished-products/", payload);
      ...
      if (imageFileRef.current) {
        ...
        await api.patch(`semi-finished-products/${newId}/`, fd);
        // fetch fresh copy with absolute image URL
        const detail = await api.get(`semi-finished-products/${newId}/`);
        saved = detail.data;
      }
  ...
  const requestSubmit = () => {
    ...
    if (window.confirm("Bạn có chắc muốn lưu bán thành phẩm này không?")) {
      handleSubmit();
    }
  };
  ...
  return (
    <div>
      ...
      {showSuccess && (
        <SuccessModal
          message="Thêm bán thành phẩm thành công!"
          onClose={() => {

...
export default function ProductionRequestFormPage({
  ...
  useEffect(() => {
    ...
    api
      ...
      .then((res) => {
        const productionRequest = res.data || {};
        setForm({
          name: productionRequest.name || "",
          request_date: productionRequest.request_date || "",
          expected_date: productionRequest.expected_date || "",
          notes: productionRequest.notes || "",
        });
        setItems(
      ...
      .catch(() => setErrors({ submit: "Không tìm thấy yêu cầu đặt hàng." }))
      .finally(() => setLoading(false));
  }, [mode, productionRequestId]);
  ...
  const parseImportedRows = async (file) => {
    ...
    return rows.map((row) => {
      ...
      const itemNotes = row["Ghi chú"] || row.notes || row.ghi_chu || "";
  ...
  const validate = () => {
    const nextErrors = {};
    if (!form.name.trim()) nextErrors.name = "Tên yêu cầu là bắt buộc";
    if (!form.request_date) nextErrors.request_date = "Ngày yêu cầu là bắt buộc";
    if (
      form.request_date &&
      form.expected_date &&
      new Date(form.expected_date) < new Date(form.request_date)
    ) {
      nextErrors.expected_date = "Ngày mong muốn phải sau ngày yêu cầu";
    }
  ...
  const saveRequest = (status) => {
    ...
    const fallbackDraftName = `Yêu cầu nháp ${new Date()
      .toISOString()
      .slice(0, 19)
      .replace("T", " ")}`;
    const payload = {
    ...
    const request =
      mode === "edit"
        ? api.put(`production-requests/${productionRequestId}/`, payload)
        : api.post("production-requests/", payload);
    ...
    request
      ...
      .catch((error) => {
        ...
        const submitMessage =
          responseData.detail ||
          responseData.name?.[0] ||
          responseData.request_date?.[0] ||
          responseData.expected_date?.[0] ||
          "Không thể lưu yêu cầu đặt hàng.";
  ...
  const requestCancel = () => {
    if (saving) return;
    if (
      window.confirm("Bạn có chắc muốn hủy biểu mẫu yêu cầu đặt hàng này không?")
    ) {
      onCancel?.();
    }
  };
  ...
  const requestSave = (status) => {
    ...
    const message =
      status === "draft"
        ? "Bạn có chắc muốn lưu nháp yêu cầu đặt hàng này không?"
        : mode === "edit"
          ? "Bạn có chắc muốn cập nhật yêu cầu đặt hàng này không?"
          : "Bạn có chắc muốn lưu yêu cầu đặt hàng này không?";
    ...
    <div>
        ...
        <div className="space-y-6">
          <div className="bg-white rounded-xl border border-gray-200 p-6">
            ...
            <div className="mb-4">
              ...
              <input
                ...
                placeholder="Nhập tên yêu cầu đặt hàng"
          ...
          <div className="bg-white rounded-xl border border-gray-200 p-6">
            <h2 className="text-base font-semibold text-gray-800 mb-4">
              Bán thành phẩm yêu cầu <span className="text-red-500">*</span>
            </h2>
      ...
      {showSuccess && (
        <SuccessModal
          message={
            mode === "edit"
              ? "Cập nhật yêu cầu đặt hàng thành công!"
              : "Thêm yêu cầu đặt hàng thành công!"
          }
          onClose={() => {

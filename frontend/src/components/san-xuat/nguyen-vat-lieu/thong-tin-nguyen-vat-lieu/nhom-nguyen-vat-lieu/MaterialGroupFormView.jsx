...
export default function MaterialGroupFormView({
  mode,
  groupId,
  onCancel,
  onSaved,
  ...
  const validate = () => {
    const e = {};
    if (!name.trim()) e.name = "Tên nhóm nguyên vật liệu là bắt buộc";
    return e;
  };
  ...
  const handleSave = async () => {
    ...
    try {
      let res;
      if (isEdit) {
        res = await api.put(`material-groups/${groupId}/`, {
          name: name.trim(),
          description: description.trim(),
          status,
        });
      } else {
        res = await api.post("material-groups/", {
          name: name.trim(),
          description: description.trim(),
        });
      }
  ...
  const requestSave = () => {
    ...
    if (
      window.confirm(
        isEdit
          ? "Bạn có chắc muốn cập nhật nhóm nguyên vật liệu này không?"
          : "Bạn có chắc muốn lưu nhóm nguyên vật liệu này không?",
      )
    ) {
      handleSave();
    }
        ...
        <>
          <div className="relative px-6 py-4 border-b border-gray-200">
            <h1 className="text-[18px] font-bold text-gray-900 uppercase pr-10">
              {`Chỉnh sửa nhóm nguyên vật liệu ${history?.code || ""}`}
            </h1>
            <button
              type="button"
              ...
              {history && (
                <div className="border border-gray-200 rounded-md p-4">
                  ...
                  <div className="flex items-center gap-2 text-sm text-gray-600 flex-wrap">
                    ...
                    <span>Thêm mới nhóm nguyên vật liệu</span>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 backdrop-blur-sm">
          <div className="bg-white rounded-2xl shadow-2xl w-80 mx-4 px-8 py-10 flex flex-col items-center">
            ...
            <p className="text-xl font-semibold italic text-gray-900 mb-6 text-center">
              {isEdit
                ? "Chỉnh sửa nhóm nguyên vật liệu thành công!"
                : "Thêm nhóm nguyên vật liệu thành công!"}
            </p>
            <button
              onClick={() => {

...
export default function EditMaterialPage({ materialId, onCancel, onSaved }) {
  ...
  useEffect(() => {
    Promise.all([
      api
        .get("material-groups/")
        .catch(() => ({ data: { material_groups: [] } })),
      api.get(`materials/${materialId}/`),
    ])
      .then(([groupsRes, materialRes]) => {
        const material = materialRes.data;
        setMaterialGroups(groupsRes.data.material_groups || []);
        setForm({
          name: material.name || "",
          group: material.group || "",
          unit: material.unit || "",
          notes: material.notes || "",
          batch_management: Boolean(material.batch_management),
      ...
      .catch(() =>
        setErrors({ submit: "Không thể tải dữ liệu nguyên vật liệu." }),
      )
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      await api.patch(`materials/${materialId}/`, fd, {
        headers: { "Content-Type": "multipart/form-data" },
      });

      const detail = await api.get(`materials/${materialId}/`);
  ...
  const requestCancel = () => {
    ...
    if (
      window.confirm(
        "Bạn có chắc muốn hủy chỉnh sửa nguyên vật liệu này không?",
      )
    ) {
      onCancel?.();
    }
  ...
  const requestSubmit = () => {
    ...
    if (
      window.confirm("Bạn có chắc muốn cập nhật nguyên vật liệu này không?")
    ) {
      handleSubmit();
    }
  };
    ...
    return (
      <div className="flex flex-col items-center justify-center py-24 gap-3">
        ...
        <p className="text-sm text-gray-500">
          Đang tải dữ liệu nguyên vật liệu...
        </p>
      </div>
    );
  }
    ...
    <div>
            ...
            <div className="grid grid-cols-2 gap-4 mb-5">
              <label className="flex items-center gap-3">
                <input
                  ...
                  onChange={(e) =>
                    setField("batch_management", e.target.checked)
                  }
              ...
              <div>
                ...
                <div className="relative">
                  <select
                    ...
                    className="w-full px-3 pr-8 py-2.5 text-sm border border-gray-200 rounded-lg appearance-none focus:outline-none focus:ring-2 focus:ring-orange-300 bg-gray-50 focus:bg-white"
                  >
      ...
      <div className="mt-8 flex justify-end items-center gap-3">
        ...
        <button
          ...
          disabled={saving || materialGroups.length === 0}

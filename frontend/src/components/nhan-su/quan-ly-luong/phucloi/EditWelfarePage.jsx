...
import { ChevronLeft, ChevronDown, RotateCcw, Loader2, Upload, X, FileText, Clock, User } from 'lucide-react'
...
export default function EditWelfarePage({ benefit: initialBenefit, onCancel, onSaved }) {
  const [form, setForm] = useState({
    name: initialBenefit.name || '',
    benefit_type: initialBenefit.benefit_type || '',
    cycle: initialBenefit.cycle || '',
    effective_from: initialBenefit.effective_from || '',
    effective_to: initialBenefit.effective_to || '',
    value: initialBenefit.value || '',
    value_unit: initialBenefit.value_unit || 'dong',
    scope: initialBenefit.scope || '',
    notes: initialBenefit.notes || '',
    status: initialBenefit.status || 'active',
  })
  const [errors, setErrors] = useState({})
  const [saving, setSaving] = useState(false)
  const [showSuccess, setShowSuccess] = useState(false)
  const [history, setHistory] = useState(initialBenefit.history || [])
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      const res = await api.patch(`benefits/${initialBenefit.id}/`, fd, {
  ...
  return (
    <div className="min-h-full bg-[#FFF6F3] p-6" style={{ fontFamily: 'Nunito Sans, sans-serif' }}>
      ...
      <h1 className="text-2xl font-bold text-gray-800 mb-6">CHỈNH SỬA CHÍNH SÁCH PHÚC LỢI</h1>
      ...
      <div className="flex gap-5 items-start">
        ...
        <div className="flex-1 min-w-0 space-y-4">
          <div className="bg-white rounded-xl border border-gray-200 p-6">
            <div className="flex items-center gap-3 mb-4 pb-3 border-b border-gray-100">
              <span className="font-mono text-xs text-orange-500 font-bold bg-orange-50 px-2 py-1 rounded">
                {initialBenefit.code}
              </span>
              <span className="text-sm text-gray-500">Mã phúc lợi</span>
            </div>
            ...
            <div className="space-y-4">
              ...
              <Field label="Trạng thái" error={errors.status}>
                <SelectWrap>
                  <select value={form.status} onChange={e => setField('status', e.target.value)} className={sel(errors.status)}>
          ...
          <div className="bg-white rounded-xl border border-gray-200 p-6">
            ...
            {initialBenefit.attachment_url && (
              <div className="mb-3 flex items-center gap-3 px-3 py-2 bg-orange-50 border border-orange-200 rounded-lg">
                <FileText size={15} className="text-orange-500 flex-shrink-0" />
                <a
                  href={initialBenefit.attachment_url}
                  target="_blank"
            ...
            <input ref={fileInputRef} type="file" multiple accept=".doc,.docx,.pdf,.jpg,.jpeg,.png" className="hidden" onChange={e => handleFiles(e.target.files)} />
          ...
          {/* Lịch sử */}
          <div className="bg-white rounded-xl border border-gray-200 p-6">
            <h3 className="text-sm font-semibold text-gray-700 mb-4 pb-3 border-b border-gray-100">Lịch sử</h3>
                        ...
                        <div key={h.id} className="flex gap-3">
                          ...
                          <div className="flex-1">
                            <div className="flex items-center gap-2 flex-wrap">
                              <span className="text-sm text-gray-800">{h.action}</span>
                              {h.old_value && h.new_value && (
                                <span className="text-xs text-gray-400">
                                  ({h.old_value} → <span className="text-orange-500 font-medium">{h.new_value}</span>)
                                </span>
                              )}
                            </div>
        ...
        <div className="w-80 flex-shrink-0 space-y-4">
          ...
          <div className="bg-white rounded-xl border border-gray-200 p-6">
            ...
            <p className="text-sm font-semibold text-gray-800 mb-4">{form.name || '—'}</p>
      ...
      {showSuccess && (
        <SuccessModal
          message="Cập nhật chính sách phúc lợi thành công!"

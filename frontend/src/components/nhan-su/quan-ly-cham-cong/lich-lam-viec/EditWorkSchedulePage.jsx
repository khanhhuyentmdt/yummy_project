...
function fromApi(s) {
  return {
    ...
    work_shift:   String(s.work_shift || ''),
    start_date:   s.start_date || '',
    end_date:     s.end_date || '',
    repeat_type:  s.repeat_type || 'once',
    days_of_week: s.days_of_week ? s.days_of_week.split(',').filter(Boolean) : [],
    notes:        s.notes || '',
...
export default function EditWorkSchedulePage({ schedule, onBack, onSaved }) {
  const [form,        setForm]       = useState(fromApi(schedule))
  const [original,   setOriginal]   = useState(fromApi(schedule))
  const [schedData,  setSchedData]  = useState(schedule)
  ...
  function toggleDay(d) {
    setForm(f => {
      ...
      return { ...f, days_of_week: cur.includes(d) ? cur.filter(x => x !== d) : [...cur, d].sort() }
    })
    setSaved(false)
  }
  ...
  async function handleSave() {
    ...
    try {
      ...
      const { data } = await api.patch(`/schedules/${schedData.id}/`, payload)
      setSchedData(data)
      setForm(fromApi(data))
      setOriginal(fromApi(data))
      onSaved(data)
      setSaved(true)
      setSuccessMsg(`Đã cập nhật lịch làm việc ${data.code} thành công!`)
    } catch (err) {
  ...
  const history = schedData.history || []

  return (
    <div className="p-6 font-nunito-sans">
      <div className="text-sm text-gray-500 mb-1">
        Nhân sự &rsaquo; Quản lý chấm công &rsaquo; Lịch làm việc &rsaquo;
        <span className="text-gray-800 font-semibold"> {schedData.code}</span>
      </div>
      <h1 className="text-2xl font-bold text-gray-800 mb-6">Chỉnh sửa lịch làm việc</h1>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-2 space-y-4">
          <div className="bg-white rounded-xl p-6 border border-gray-100 shadow-sm space-y-5">
            ...
            <div>
              <label className="block text-sm font-medium text-gray-700 mb-1">Mã lịch</label>
              <input className="w-full px-3 py-2 border border-gray-100 rounded-lg text-sm bg-gray-50 text-gray-400"
                value={schedData.code} disabled />
            </div>
              ...
              <div className="flex gap-4">
                {[['once', 'Một lần'], ['weekly', 'Hằng tuần'], ['monthly', 'Hằng tháng']].map(([val, label]) => (
                  <label key={val} className="flex items-center gap-2 cursor-pointer">
                    <input type="radio" name="edit_repeat" value={val}
                      checked={form.repeat_type === val} onChange={() => setField('repeat_type', val)}
